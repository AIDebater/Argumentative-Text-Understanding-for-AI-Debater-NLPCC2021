...
class BaselineLSTMModel(nn.Module):
    ...
    def __init__(self, args, bertModel):
        ...
        info = f"""[model info] model: BaselineLSTMModel
    ...
    def forward(self, embedder_input, input_mask, seq_lens, bio_tags, type_idx):
        ...
        expanded_lstm_embedding = lstm_embedding.unsqueeze(2).expand([-1, -1, num_sents, -1])
        expanded_lstm_embedding_t = lstm_embedding.unsqueeze(1).expand([-1, num_sents, -1, -1])
        cross_feature = torch.cat((expanded_lstm_embedding, expanded_lstm_embedding_t), dim=3)
